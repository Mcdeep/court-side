...
function PlayersPage() {
  ...
  const [linkingMember, setLinkingMember] = useState<{ id: Id<"members">; name: string } | null>(
    null,
  );
  const [removeError, setRemoveError] = useState<{ id: Id<"members">; message: string } | null>(
    null,
  );
  ...
  const data = useQuery(api.participants.listByOrg, org ? { organizationId: org._id } : "skip");
    ...
    <div className="w-full px-10 py-8">
      ...
      <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card overflow-hidden mb-6">
        ...
        {filteredRoster.length === 0 ? (
          <div className="px-5 py-12 text-center text-ink-mute text-sm">
            {search
              ? "No roster members match."
              : "No members registered yet. Add or import your club’s regulars."}
          </div>
        ) : (
          <>
            ...
            {filteredRoster.map((m) => (
              ...
              >
                ...
                <div className="text-right tnum text-sm text-ink-mute">
                  {m.startingPoints ?? "—"}
                </div>
                ...
                <div className="flex justify-end gap-1">
                  {m.userId ? (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => unlinkMember({ memberId: m._id })}
                    >
                      Unlink
                    </Button>
                  ) : (
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setLinkingMember({ id: m._id, name: m.name })}
                    >
                      Link
                    </Button>
                  )}
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setRemoveError({
                          id: m._id,
                          message: cleanConvexError(e, "Could not remove"),
                        });
                      }
                    }}
                  />
      ...
      <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card overflow-hidden mb-6">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-zinc-100">
          <div className="font-semibold text-[15px]">
            Played before{" "}
            <span className="text-ink-mute font-normal">· {filteredMembers.length}</span>
          </div>
        </div>
        ...
        {filteredMembers.length === 0 ? (
          <div className="px-5 py-12 text-center text-ink-mute text-sm">
            {search
              ? "No members match."
              : "No history yet. Players appear here after joining a tournament."}
          </div>
        ) : (
          <>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[12px] text-ink-mute">
                      Walk-in · {w.count} tournament{w.count !== 1 ? "s" : ""}
                    </div>
                  </div>
                </div>
              ))}
...
function RatingEditor({
  ...
  if (editing) {
    return (
      <div className="flex items-center gap-1">
        ...
        <button
          onClick={save}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-accent-dark hover:bg-accent-soft shrink-0"
        >
  ...
  return (
    <button
      onClick={() => {
        setValue(skillRating !== undefined ? String(skillRating) : "");
        setEditing(true);
      }}
