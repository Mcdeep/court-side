...
function TournamentDetailPage() {
  ...
  const finalsComplete =
    !isDouble ||
    (rounds.some((round) => round.stage === "final") &&
      rounds
        .filter((round) => round.stage === "final")
        .every((round) => round.state === "completed"));
  ...
  return (
    <div className="w-full px-4 sm:px-10 py-5 sm:py-7">
          ...
          {tournament.state === "in_progress" && (
            <>
              <Button variant="outline" size="md" icon="screen" asChild>
                <a href={`/kiosk/${tournamentId}`} target="_blank" rel="noreferrer">
                  Kiosk
                </a>
              </Button>
      ...
      {tab === "participants" && (
        <>
          {isDouble && (
            <DoubleAmericanoGroups
              tournamentId={tid}
              participants={participants}
              mode={tournament.groupSplitMode}
              canEdit={canAddPlayer && rounds.length === 0}
            />
          )}
      ...
      <Sheet open={showLeaderboard} onOpenChange={setShowLeaderboard}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader>
            <SheetTitle className="font-display font-bold text-[20px] tracking-tight">
              Leaderboard
            </SheetTitle>
          </SheetHeader>
