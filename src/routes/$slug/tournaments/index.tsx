...
function TournamentsPage() {
  ...
  const counts = {
    ...
    draft: tournaments.filter((t) => normaliseStatus(t.state as TournamentStatus) === "draft")
      .length,
    completed: tournaments.filter(
      (t) => normaliseStatus(t.state as TournamentStatus) === "completed",
    ).length,
  };

  const rows = tournaments.filter(
    (t) => filter === "all" || normaliseStatus(t.state as TournamentStatus) === filter,
  );
        ...
        <div className="min-w-[640px]">
          ...
          {rows.length === 0 && (
            <div className="px-5 py-16 flex flex-col items-center gap-4 text-center">
              ...
              {filter === "live" && (
                <Button variant="primary" size="md" icon="plus" onClick={goNew}>
                  New tournament
                </Button>
              )}
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  >
                    <Icon
                      name={t.format === "knockout" ? "trophy" : "shuffle"}
                    ...
                    (() => {
                      ...
                      return (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="tnum text-[12px] text-ink-mute w-9 text-right">
                            {pct}%
                          </span>
                        </div>
                      );
                ...
                <div className="w-28 flex items-center justify-end gap-1 pr-1">
                  ...
                  <Icon
                    name="chevR"
                    className="w-4 h-4 text-zinc-300 group-hover:text-ink group-hover:translate-x-0.5 transition-all"
                  />
                </div>
              </button>
            );
...
function Stat({
  ...
  return (
    <div
      className={`rounded-2xl p-4 ring-1 shadow-card ${accent ? "bg-ink text-paper ring-ink" : "bg-white ring-zinc-200/80"}`}
    >
      <div className="flex items-center justify-between">
        <span
          className={`text-[12px] font-semibold uppercase tracking-wide whitespace-nowrap ${accent ? "text-paper/60" : "text-ink-mute"}`}
        >
          {label}
        </span>
        <span
          className={`w-7 h-7 rounded-lg flex items-center justify-center ${accent ? "bg-accent text-ink" : "bg-zinc-100 text-zinc-400"}`}
        >
          <Icon name={icon as any} className="w-4 h-4" />
        </span>
      </div>
      <div className="mt-2 font-display font-bold text-[30px] leading-none tnum">{value}</div>
      <div className={`text-[12.5px] mt-1 ${accent ? "text-paper/55" : "text-ink-mute"}`}>
        {sub}
      </div>
    </div>
  );
