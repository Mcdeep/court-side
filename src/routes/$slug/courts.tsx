...
function CourtsPage() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-10 py-8">
      {showAdd && (
        <VenueModal orgId={org._id} onClose={() => setShowAdd(false)} />
      )}

      {/* Header */}
      <div className="flex items-end justify-between gap-4 mb-7">
        <div>
          <div className="text-ink-mute text-[13px] font-semibold mb-2 capitalize">
            {org.name}
          </div>
...
const INPUT_CLS =
  "w-full h-10 px-3.5 rounded-xl bg-white ring-1 ring-zinc-200 text-sm focus:outline-none focus:ring-2 focus:ring-accent-dark/40 transition-all";
...
function Stat({
  ...
  return (
    <div className="rounded-2xl p-4 bg-white ring-1 ring-zinc-200/80 shadow-card">
      ...
      <div className="mt-2 font-display font-bold text-[30px] leading-none tnum">
        {value}
      </div>
    </div>
  );
