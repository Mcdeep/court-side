...
function ManagePage() {
  ...
  if (!pin || invalidPin) {
    return (
      <PinGate tournamentId={tournamentId} wrongPin={invalidPin} onUnlocked={handleUnlocked} />
    );
  }
  ...
  const currentRound =
    rounds.find((r) => r.state === "in_progress") ?? rounds.find((r) => r.state === "pending");
...
function PinGate({
  ...
  const publicInfo = useQuery(api.tournaments.getPublic, {
      ...
      <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-lg p-8 max-w-sm w-full">
        <div className="text-center mb-6">
          ...
          <h1 className="text-lg font-bold text-zinc-900 truncate">
            {publicInfo?.name ?? "Manage tournament"}
          </h1>
