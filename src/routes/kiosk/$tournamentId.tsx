...
function KioskPage() {
  ...
  const currentRound =
    rounds?.find((r) => r.state === "in_progress") ??
  ...
  const matches = useQuery(
    api.matches.listByRound,
    currentRound ? { roundId: currentRound._id } : "skip",
  );
  const nextMatches = useQuery(
    api.matches.listByRound,
    nextRound ? { roundId: nextRound._id } : "skip",
  );

  const isCompleted = tournament?.state === "completed";
  const history = useQuery(
    api.matches.historyByTournament,
    isCompleted ? { tournamentId: tournamentId as Id<"tournaments"> } : "skip",
  );
  ...
  const sortedNextMatches = nextMatches
    ? [...nextMatches].sort((a, b) => a.courtNumber - b.courtNumber)
    : [];
  ...
  return (
    <div className="h-screen bg-ink text-paper flex flex-col overflow-hidden select-none">
      ...
      <header className="flex items-center justify-between px-8 h-24 border-b border-white/8 shrink-0 gap-4">
        <div className="flex items-center gap-4 min-w-0">
          ...
          {isLive && tournament.roundDurationMs && currentRound?.startedAt && (
            <RoundTimer
              durationMs={tournament.roundDurationMs}
          ...
          <aside className="min-h-0">
            <section className="rounded-[28px] bg-white/[0.04] ring-1 ring-white/10 p-6 flex flex-col h-full min-h-0">
              <h2 className="font-display font-bold text-[19px] tracking-tight flex items-center gap-2 mb-2 shrink-0 whitespace-nowrap">
                ...
                {tournament.americanoVariant === "double"
                  ? "Double Americano standings"
                  : "Leaderboard · Top 10"}
              </h2>
              {leaderboard.length === 0 ? (
                <div className="flex-1 flex items-center justify-center text-paper/30 text-lg">
                  No scores yet
                </div>
              ) : tournament.americanoVariant === "double" ? (
                <DoubleStandings rows={leaderboard} />
              ) : (
      ...
      {!isCompleted && sortedNextMatches.length > 0 && (
        <TickerMarquee matches={sortedNextMatches} />
      )}
