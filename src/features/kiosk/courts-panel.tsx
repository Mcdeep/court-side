    ...
    <div className="min-h-0 flex flex-col h-full">
        ...
        {pageCount > 1 && (
          <div className="flex items-center gap-1.5">
            {Array.from({ length: pageCount }).map((_, i) => (
              <span
                key={i}
                className={`h-1.5 rounded-full transition-all ${i === pageIdx ? "w-6 bg-accent" : "w-1.5 bg-white/20"}`}
              />
            ))}
          </div>
        )}
      ...
      {matches.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 gap-4">
          ...
          <span className="text-paper/30 text-lg">
            {isPreMatch ? "Waiting for players…" : "No matches yet"}
          </span>
        </div>
      ) : (
