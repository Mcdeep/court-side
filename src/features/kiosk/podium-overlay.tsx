...
export function PodiumOverlay({
  ...
  useEffect(() => {
    setRevealed(0);
    const timers = rankKey
      .split(",")
      .filter(Boolean)
      .map((rank, index) =>
        setTimeout(
          () => setRevealed((current) => Math.max(current, Number(rank))),
          index * REVEAL_MS,
        ),
      );
  ...
  return (
    ...
    >
      <div className="text-center">
        <div className="text-[13px] font-bold uppercase tracking-[0.2em] text-accent mb-2">
          Tournament complete
        </div>
              ...
              >
                {entries.map((entry) => (
                  ...
                  >
                    ...
                    <div className="font-display font-bold text-xs sm:text-[18px] leading-tight break-words">
                      {entry.players.map((player) => player.displayName).join(" / ")}
                    </div>
                    <div className="font-mono tnum text-[14px] text-paper/50">
                      {entry.points} pts
                    </div>
                  </div>
                ))}
                ...
                >
                  <span className="font-display font-bold text-[28px]">
                    {entries.length > 1 ? "=" : ""}
                    {rank}
                  </span>
                </div>
