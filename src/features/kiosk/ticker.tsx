...
export function TickerMarquee({ matches }: { matches: Match[] }) {
  const ref = useAutoScroll<HTMLDivElement>(
    {
      axis: "x",
      activeClass: "is-overflowing",
      shiftVar: "--ticker-shift",
      durationVar: "--ticker-duration",
      minDurationSec: 16,
      speedDivisor: 40,
      extraPadding: 40,
    },
        ...
        <div className="ticker-track flex items-center gap-10 whitespace-nowrap">
          {matches.map((m) => {
            ...
            return (
              <span
                key={m._id}
                className="inline-flex items-center gap-3 text-[16px] font-semibold text-paper/80"
              >
                {a1} / {a2}
                <span className="text-paper/30">vs</span>
                {b1} / {b2}
                <span className="text-[12px] font-bold uppercase tracking-wide text-paper/40">
                  Court {m.courtNumber}
                </span>
              </span>
            );
