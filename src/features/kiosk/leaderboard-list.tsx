...
function BoardRow({ entry }: { entry: LeaderboardEntry }) {
  ...
  const tied =
    entry.finalPlacement !== undefined ||
    (entry.groupRank !== undefined ? entry.groupTied : entry.tied);
  const medal =
    rank === 1
      ? "bg-accent text-ink"
      : rank === 2
  ...
  return (
    <div className="grid grid-cols-[36px_1fr_54px] items-center gap-3 py-1.5 border-b border-white/6 last:border-0">
      <span
        className={`w-8 h-8 rounded-lg flex items-center justify-center font-display font-bold text-[15px] tnum ${medal || "text-paper/40"}`}
      >
        {tied ? "=" : ""}
        {rank}
      </span>
  ...
  const ref = useAutoScroll<HTMLDivElement>(
    {
      ...
      shiftVar: "--board-shift",
      durationVar: "--board-duration",
      minDurationSec: 10,
      speedDivisor: 12,
    },
    [rows],
  ...
  return (
    <div ref={ref} className="board-mask scrollbar-none overflow-hidden flex-1 min-h-0">
      <div className="board-track">
        {rows.map((row) => (
          <BoardRow key={row._id} entry={row} />
        ))}
      </div>
    </div>
  );
