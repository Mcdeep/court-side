    ...
    <>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="font-display font-bold text-[16px] leading-tight">
                  {placementLabel
                    ? `${placementLabel} · Court ${match.courtNumber}`
                    : `Court ${match.courtNumber}`}{" "}
                  · Score
                </div>
                <div className="text-[12px] text-ink-mute font-normal">
                  {shared
                    ? `Tap one side — the other fills to ${pointsToWin}`
                    : timeBased
                      ? "Time's up — enter each side's points, most wins"
                      : "Tap each side's score"}
                </div>
              </div>
            </div>
          }
            ...
            <>
              {previousScore && (
                <p className="text-sm text-ink-mute">
                  The previous result is filled in below. Save it unchanged to restore it, or select
                  the corrected scores.
                </p>
              )}
              {match.finalMatchIndex !== undefined && (
                <p className="text-sm text-ink-mute">
                  Finals require a winner, so tied scores are not accepted. Final points do not
                  change group standings.
                </p>
              )}
              {error && (
      ...
      <button
        ...
        className={`text-left w-full rounded-xl p-3 ring-1 transition-all hover:shadow-card hover:-translate-y-px
          ${
            live
              ? "bg-accent-soft ring-accent-dark/30"
              : final
      ...
      >
        <div className="flex items-center justify-between mb-2">
          ...
          {live && (
            <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-ink">
              <span
                className="relative inline-block w-1.5 h-1.5 rounded-full bg-ink live-ping"
                style={{ color: "oklch(0.19 0.012 264)" }}
              />
              Live
            </span>
          )}
          {final && (
            <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-zinc-400">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between gap-2 text-ink">
            ...
            <span
              className={`font-mono tnum text-[15px] shrink-0 font-bold
              ${
...
function NumberGrid({
  ...
  return (
    <div
      className={`rounded-2xl p-3 ring-1 min-w-0 ${highlight ? "bg-accent-soft ring-accent-dark/30" : "bg-zinc-50 ring-zinc-200"}`}
    >
