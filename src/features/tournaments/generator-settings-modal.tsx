...
export function GeneratorSettingsModal({
  tournament,
  onClose,
}: {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!timeBased && (!Number.isInteger(parsed) || parsed < 1)) {
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field label="Scoring">
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setScoringMode("first_to")}
            ...
            <button
              type="button"
              onClick={() => setScoringMode("shared_total")}
              className={`flex-1 h-9 rounded-lg text-sm font-semibold ring-1 transition-colors
            ...
            <button
              type="button"
              onClick={() => setScoringMode("time_based")}
              className={`flex-1 h-9 rounded-lg text-sm font-semibold ring-1 transition-colors
