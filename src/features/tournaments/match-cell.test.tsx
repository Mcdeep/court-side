...
const match: Match = {
  _id: "match" as Id<"matches">,
  _creationTime: 0,
  roundId: "round" as Id<"rounds">,
  pairAId: "a" as Id<"pairs">,
  pairBId: "b" as Id<"pairs">,
  courtNumber: 1,
  state: "completed",
  pairA: { participantA: null, participantB: null },
  pairB: { participantA: null, participantB: null },
};

function enterPreviousScore(a: string, b: string) {
  fireEvent.change(screen.getByRole("spinbutton", { name: "Previous score for side A" }), {
    target: { value: a },
  });
  fireEvent.change(screen.getByRole("spinbutton", { name: "Previous score for side B" }), {
    target: { value: b },
  });
...
test("repairs a missing score through the normal editor before applying a correction", async () => {
  ...
  await waitFor(() =>
    expect(saveResult).toHaveBeenCalledWith({
      ...
      scoreB: 8,
      pin: "1234",
      previousScore: { scoreA: 10, scoreB: 8 },
    }),
  );
...
test("requires both previous scores and accepts zero as a real score", async () => {
  ...
  await waitFor(() =>
    expect(saveResult).toHaveBeenCalledWith({
      ...
      scoreB: 8,
      pin: undefined,
      previousScore: { scoreA: 0, scoreB: 8 },
    }),
  );
});
...
test("keeps a failed save open and shows its error", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Save" }));
  await waitFor(() =>
    expect(screen.getByRole("alert").textContent).toContain("Score could not be saved"),
  );
