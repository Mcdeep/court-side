...
export function TiebreakOrderField({ value, onChange, disabled = false }: {
  value: readonly Tiebreak[]; onChange: (value: Tiebreak[]) => void; disabled?: boolean
}) {
        ...
        {value.map((criterion, index) => (
          <li key={criterion} className="flex items-center gap-3 py-2">
            ...
            <Button type="button" variant="ghost" size="icon-sm" disabled={disabled || index === 0}
              aria-label={`Move ${TIEBREAK_LABELS[criterion]} up`} onClick={() => move(index, -1)}>↑</Button>
            <Button type="button" variant="ghost" size="icon-sm" disabled={disabled || index === value.length - 1}
              aria-label={`Move ${TIEBREAK_LABELS[criterion]} down`} onClick={() => move(index, 1)}>↓</Button>
          </li>
        ))}
      </ol>
      <p className="mt-2 text-xs text-ink-mute">Players still level share a position. The order locks when the tournament is completed.</p>
    </Field>
  )
}
