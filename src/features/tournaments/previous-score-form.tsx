...
export function PreviousScoreForm({ match, onConfirm }: { match: Match; onConfirm: (score: PreviousScore) => void }) {
  ...
  function confirm(e: React.FormEvent) {
    e.preventDefault()
    if (!a.trim() || !b.trim()) { setError('Enter both previous scores'); return }
  ...
  return (
    <form onSubmit={confirm} noValidate className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-mute">This completed match has no saved result. Enter the score previously counted in standings. You can then keep it or correct it.</p>
      </div>
      <Field label={pairNames(match.pairA).join(' / ')}>
        <Input type="number" min={0} step={1} value={a} onChange={e => setA(e.target.value)} aria-label="Previous score for side A" />
      </Field>
      <Field label={pairNames(match.pairB).join(' / ')}>
        <Input type="number" min={0} step={1} value={b} onChange={e => setB(e.target.value)} aria-label="Previous score for side B" />
      </Field>
