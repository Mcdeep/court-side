...
export function ManageSchedule({
  ...
  const canFinish =
    !isDouble || (finals.length > 0 && finals.every((round) => round.state === "completed"));
      ...
      <div>
        ...
        <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card p-8 flex flex-col items-center text-center">
          ...
          <p className="text-ink-mute text-sm mt-1">
            {isDouble
              ? "The organiser must assign two groups of eight before the group stage can be generated."
          ...
          {!canGenerate && (
            <p className="text-[12.5px] text-red-500 mt-2">
              {isDouble
                ? "Ask the organiser to assign both groups"
    ...
    <div>
      ...
      {(!isPreGenerated || (isDouble && finals.length === 0)) && !locked && (
        <div className="flex flex-col items-center gap-2 mt-6 rounded-2xl border-2 border-dashed border-zinc-300 p-5">
          ...
          {!isDouble && checkedInCount < 4 && (
            <p className="text-[12.5px] text-ink-mute text-center">
      ...
      {!locked && (
        <Button
          ...
          onClick={() => setShowFinish(true)}
          disabled={!canFinish}
          title={!canFinish ? "Complete all crossover finals first" : undefined}
        >
          Finish tournament
        </Button>
      )}
  ...
  return (
    <div>
      <div className="flex items-center gap-2.5 mb-1">
        ...
        {round.state !== "pending" && (
          <span className="text-[12px] text-ink-mute tnum ml-auto">
