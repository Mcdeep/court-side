...
const entry = (name: string, rank: number, tied = false, wins = 1): LeaderboardEntry => ({
  _id: name as Id<"participants">,
  participantId: name as Id<"participants">,
  participantIds: [name as Id<"participants">],
  players: [{ displayName: `Player ${name}` }],
  points: 20,
  wins,
  losses: 1,
  played: wins + 1,
...
test("lets the organiser change the priority using accessible buttons", () => {
  ...
  expect(screen.getByRole("button", { name: "Move Matches won up" }).hasAttribute("disabled")).toBe(
    true,
  );
  expect(
    screen.getByRole("button", { name: "Move Head-to-head down" }).hasAttribute("disabled"),
  ).toBe(true);
});
...
test("the standings caption follows a wins-first ranking order", () => {
  ...
  expect(
    screen.getByText(
      "Matches won \u2192 Total points \u2192 Point difference \u2192 Head-to-head. Equal results share a position.",
    ),
  ).toBeTruthy();
});
...
test("locked settings cannot reorder the rules", () => {
  render(
    <TiebreakOrderField
      value={DEFAULT_TIEBREAK_ORDER}
      onChange={() => {
        throw new Error("Locked setting changed");
      }}
      disabled
    />,
  );
...
test("shows official shared ranks and point difference, even after sorting another column", () => {
  ...
  expect(
    within(screen.getByRole("row", { name: /Player A/ })).getByLabelText("Position 1"),
  ).toBeTruthy();
  ...
  expect(
    within(screen.getByRole("row", { name: /Player D/ })).getByLabelText("Position 4"),
  ).toBeTruthy();
});
...
test("fallback standings explain the rating basis and how to restore missing results", () => {
  render(
    <StandingsTab
      leaderboard={[{ ...entry("A", 1), pointDiff: null, tiebreaksUnavailable: true }]}
    />,
  );
...
test("the podium includes every player sharing third place", () => {
  ...
  render(
    <PodiumOverlay
      ...
      top3={[entry("A", 1), entry("B", 2), entry("C", 3, true), entry("D", 3, true)]}
      onDismiss={() => {}}
    />,
  );
...
test("the podium can share first place without inventing second or third place", () => {
  ...
  render(
    <PodiumOverlay
      tournamentName="Final"
      top3={[entry("A", 1, true), entry("B", 1, true), entry("C", 1, true)]}
      onDismiss={() => {}}
    />,
  );
