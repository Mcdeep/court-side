...
export function ScheduleTab({
  ...
  const groupPhaseComplete =
    groupRounds.length > 0 && groupRounds.every((round) => round.state === "completed");
  const groupsReady =
    participants.filter((p) => p.group === 1).length === 8 &&
    participants.filter((p) => p.group === 2).length === 8;
  const canGenerate = isDouble
    ? rounds.length === 0
      ? groupsReady
      : groupPhaseComplete && finals.length === 0
    : isPreGenerated || (checkedInCount >= 4 && lastRoundScored);
    ...
    return (
      <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card p-12 flex flex-col items-center text-center">
        ...
        <p className="text-ink-mute text-sm mt-1 max-w-xs">
          {isDouble
            ? "Assign two groups of eight on the Participants tab, then generate the group stage."
        ...
        {!canGenerate && (
          <p className="text-[12.5px] text-red-500 mt-2">
            {isDouble
              ? "Assign exactly 8 players to each group first"
              : "Check in at least 4 players first"}
          </p>
        )}
      </div>
    );
  ...
  return (
    <div>
      ...
      {showResetFinals && (
        <ConfirmDialog
          title="Reset crossover finals?"
          body="Final matches and scores will be deleted. Group results stay intact, so you can correct them and regenerate the finals."
          confirmLabel="Reset finals"
          danger
          onConfirm={async () => {
            await resetFinals({ tournamentId: tournament._id });
            setShowResetFinals(false);
          }}
          onCancel={() => setShowResetFinals(false)}
        />
      )}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card p-4">
        ...
        <div className="flex items-center flex-wrap gap-2">
          <Button variant="outline" size="md" icon="gear" onClick={() => setShowSettings(true)}>
            Settings
          </Button>
          <Button variant="ghost" size="md" icon="reset" onClick={() => setShowReset(true)}>
            Reset
          </Button>
          {isDouble &&
            finals.length > 0 &&
            ...
            tournament.state !== "archived" && (
              <Button
                ...
                size="md"
                icon="reset"
                onClick={() => setShowResetFinals(true)}
              >
                Reset finals
              </Button>
            )}
      ...
      <div className="space-y-6">
        {rounds.map((round) => (
          <RoundRow
            ...
            blocked={rounds.some(
              (r) => r.roundNumber < round.roundNumber && r.state !== "completed",
            )}
            pin={pin}
          />
        ))}
        ...
        <div className="flex flex-col items-center justify-center gap-2 mt-6 rounded-2xl border-2 border-dashed border-zinc-300 p-6">
          ...
          {!isDouble && checkedInCount < 4 && (
            <p className="text-[12.5px] text-ink-mute">
              Check in at least 4 players before generating the next round
            </p>
          )}
          {!isDouble && checkedInCount >= 4 && !lastRoundScored && (
            <p className="text-[12.5px] text-ink-mute">
              Score and end round {lastRound?.roundNumber} before generating the next
            </p>
          )}
        </div>
      )}
...
function RoundRow({
  ...
  const doneCount =
    matches?.filter((m) => m.state === "completed" || m.state === "disputed").length ?? 0;
  ...
  async function handleStart() {
    ...
    try {
      ...
      setError(
        (e as { data?: string })?.data ?? "Score the previous round before starting this one",
      );
    } finally {
      setWorking(false);
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        <div className="flex items-center flex-wrap gap-2.5">
          <span className="font-display font-bold text-[15px]">
            {round.stage === "final"
              ? `Crossover finals · Round ${round.roundNumber}`
              : round.stage === "group"
                ? `Group stage · Round ${round.roundNumber}`
                : `Round ${round.roundNumber}`}
          </span>
          <span
            className={`text-[10px] font-bold uppercase tracking-wide px-2 h-5 inline-flex items-center rounded-full
        ...
        <div className="flex items-center gap-2">
          ...
          {round.state === "in_progress" && !completeError && !allDone && (
            <span className="text-[12px] text-ink-mute font-medium">
              {matches.length - doneCount} match{matches.length - doneCount !== 1 ? "es" : ""}{" "}
              remaining
            </span>
          )}
          {round.state === "completed" && (
