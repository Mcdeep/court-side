...
vi.mock("convex/react", () => ({
  useMutation: (reference: FunctionReference<"mutation">) =>
    getFunctionName(reference) === "ratings:setSkillRating" ? save : other,
}));
afterEach(cleanup);
beforeEach(() => {
...
const participant: Participant = {
  _id: "p" as Id<"participants">,
  _creationTime: 0,
  tournamentId: "t" as Id<"tournaments">,
  memberId: "m" as Id<"members">,
  resolvedUserId: "u" as Id<"users">,
  isWalkIn: true,
  walkInName: "Linked later",
  entryType: "solo",
  rating: undefined,
  user: null,
};
...
test("saves a newly linked player rating to the account rating", async () => {
  ...
  await waitFor(() =>
    expect(save).toHaveBeenCalledWith({ organizationId: "org", userId: "u", skillRating: 4.25 }),
  );
