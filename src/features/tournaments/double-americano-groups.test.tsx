...
beforeEach(() => {
  useMutation
    .mockReset()
    .mockImplementation((reference: FunctionReference<"mutation">) =>
      getFunctionName(reference) === "participants:assignGroups" ? assignGroups : swapGroups,
    );
...
function player(index: number, rating: number | undefined, group?: 1 | 2): Participant {
  return {
    _id: `p${index}` as Id<"participants">,
    _creationTime: index,
    tournamentId: "t" as Id<"tournaments">,
    entryType: "solo",
    checkedIn: false,
    walkInName: `Player ${index}`,
    isWalkIn: true,
    rating,
    group,
...
test("manual selection swaps one player from each group", async () => {
  ...
  await waitFor(() =>
    expect(swapGroups).toHaveBeenCalledWith({
      tournamentId: "t",
      participantAId: "p0",
      participantBId: "p8",
    }),
