...
export function StandingsTab({
  ...
  tiebreakOrder?: readonly Tiebreak[];
}) {
  ...
  const columns: { field: SortField; label: string }[] = [
    { field: "played", label: "GP" },
    { field: "wins", label: "Won" },
    { field: "losses", label: "Lost" },
    ...(showDifference ? [{ field: "pointDiff" as const, label: "Diff" }] : []),
    ...
    <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card overflow-hidden">
      {leaderboard.some((entry) => entry.tiebreaksUnavailable) && (
        <p className="px-5 py-3 text-sm text-ink-mute border-b border-zinc-100">
          Some older match results are missing. Standings and rating awards use total points until
          they are restored. Open a missing match score in the schedule to restore it.
        </p>
      )}
      {showDifference && (
        <p className="px-5 py-3 text-xs text-ink-mute border-b border-zinc-100">
          {getRankingOrder(tiebreakOrder)
            .map((rule) => TIEBREAK_LABELS[rule])
            .join(" → ")}
          . Equal results share a position.
        </p>
      )}
        ...
        <TableHeader>
          <TableRow>
            <TableHead
              className="w-16 pl-5"
              aria-sort={
                sortField === "rank" ? (sortDir === "asc" ? "ascending" : "descending") : undefined
              }
            >
              <SortHeader
                label="#"
            ...
            <TableHead className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">
              Player
            </TableHead>
            {columns.map(({ field, label }) => (
              <TableHead
                key={field}
                className="w-16 last:pr-5"
                aria-sort={
          ...
          {sorted.map((entry) => (
            <TableRow key={entry._id} className={entry.rank <= 3 ? "bg-accent-soft/40" : ""}>
              <TableCell className="pl-5">
                <span
                  aria-label={
                    entry.tied ? `Tied for position ${entry.rank}` : `Position ${entry.rank}`
                  }
              ...
              <TableCell>
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <span className="font-semibold text-sm">
                    {entry.players.map((player) => player.displayName).join(" / ")}
                  </span>
                </div>
              </TableCell>
              ...
              {showDifference && (
                <TableCell className="text-right tnum text-ink-mute">
                  {entry.pointDiff === null
                    ? "—"
                    : entry.pointDiff > 0
                      ? `+${entry.pointDiff}`
                      : String(entry.pointDiff).replace("-", "−")}
                </TableCell>
              )}
              <TableCell className="text-right pr-5 tnum font-mono font-bold text-[15px]">
                {entry.points}
              </TableCell>
            </TableRow>
          ))}
  ...
  return (
    <div className="space-y-5">
      {finalsComplete && (
        <div className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card overflow-hidden">
          <p className="px-5 py-3 text-sm text-ink-mute border-b border-zinc-100">
            Finals decide shared placements; partners share each final placement. Points, wins,
            losses, and difference remain group-stage statistics.
          </p>
          <PlacementTable
            rows={[...leaderboard].sort(
              (a, b) => (a.finalPlacement ?? 99) - (b.finalPlacement ?? 99),
            )}
          />
        </div>
      )}
      {!finalsComplete && (
        <p className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card px-5 py-3 text-xs text-ink-mute">
          {getRankingOrder(tiebreakOrder)
            .map((rule) => TIEBREAK_LABELS[rule])
            .join(" → ")}
          . A random draw resolves any tie still equal after all criteria for final seeding.
        </p>
      )}
      <div className="grid gap-5 xl:grid-cols-2">
        {groups.map(({ group, rows }) => (
          <div
            key={group}
            className="bg-white rounded-2xl ring-1 ring-zinc-200/80 shadow-card overflow-hidden"
          >
            <h3 className="font-display font-bold text-[17px] px-5 py-3 border-b border-zinc-100">
              Group {group}
            </h3>
...
function RankCell({ rank, tied }: { rank: number; tied: boolean }) {
  return (
    <TableCell className="pl-5">
      <span
        aria-label={tied ? `Tied for position ${rank}` : `Position ${rank}`}
        className={`tnum font-display font-bold text-[15px] min-w-8 h-8 px-1 rounded-lg inline-flex items-center justify-center ${rank === 1 ? "bg-accent text-ink" : rank <= 3 ? "bg-ink text-paper" : "text-ink-mute"}`}
      >
        {tied ? "=" : ""}
        {rank}
      </span>
...
function StatsCells({ entry }: { entry: LeaderboardEntry }) {
  return (
    <>
      ...
      <TableCell className="text-right pr-5 tnum font-mono font-bold">{entry.points}</TableCell>
    </>
  );
}
...
function GroupTable({ group, rows }: { group: 1 | 2; rows: LeaderboardEntry[] }) {
  return (
    <Table aria-label={`Group ${group} standings`} className="min-w-[500px]">
      <TableHeader>
        <TableRow>
          <TableHead className="w-16 pl-5">#</TableHead>
          <TableHead>Player</TableHead>
          <TableHead className="text-right">GP</TableHead>
          <TableHead className="text-right">Won</TableHead>
          <TableHead className="text-right">Lost</TableHead>
          <TableHead className="text-right">Diff</TableHead>
          <TableHead className="text-right pr-5">Pts</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {rows.map((entry) => (
          <TableRow key={entry._id}>
            <RankCell rank={entry.groupRank ?? 0} tied={entry.groupTied ?? false} />
            <PlayerCell entry={entry} />
            <StatsCells entry={entry} />
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
function PlacementTable({ rows }: { rows: LeaderboardEntry[] }) {
  return (
    <Table aria-label="Final placements" className="min-w-[380px]">
      <TableHeader>
        <TableRow>
          <TableHead className="w-16 pl-5">Place</TableHead>
          <TableHead>Player</TableHead>
          <TableHead className="pr-5">Group finish</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {rows.map((entry) => (
          <TableRow key={entry._id}>
            <RankCell rank={entry.finalPlacement ?? 0} tied />
            <PlayerCell entry={entry} />
            <TableCell className="pr-5 text-ink-mute">
              Group {entry.group} · #{entry.groupRank}
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
...
function SortHeader({
  ...
  return (
    ...
    >
      {label}
      <span aria-hidden className={`text-[9px] ${active ? "opacity-100" : "opacity-0"}`}>
        {sortDir === "desc" ? "▼" : "▲"}
      </span>
    </button>
  );
