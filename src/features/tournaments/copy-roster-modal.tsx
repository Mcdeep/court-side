...
export function CopyRosterModal({ tournamentId, organizationId, existingMemberIds, onClose }: {
    ...
    >
      <div className="space-y-3">
        ...
        {!sourceId ? (
          <div className="max-h-72 overflow-y-auto -mx-1 space-y-0.5">
            {tournaments === undefined ? (
              ...
              <div className="text-center py-6 text-ink-mute text-sm">No other tournaments to copy from.</div>
            ) : otherTournaments.map(t => (
              <button key={t._id} onClick={() => setSourceId(t._id)}
          ...
          <>
            <button onClick={() => setSourceId(null)}
        ...
        <div className="flex gap-2 pt-1">
          ...
          {sourceId && (
            <Button variant="primary" className="flex-1" onClick={copyRoster} disabled={working || toCopy.length === 0}>
              {working ? 'Copying…' : `Copy ${toCopy.length || ''} player${toCopy.length !== 1 ? 's' : ''}`}
            </Button>
          )}
        </div>
      </div>
