...
export function CreateOrgDialog({
  ...
  const assignMembership = useMutation(api.memberships.assign);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const organizationId = await createOrg({ name: name.trim(), slug: slug.trim() });
      if (adminUserId) {
        await assignMembership({ organizationId, userId: adminUserId, role: "admin" });
      }
      resetAndClose();
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          <DialogTitle className="font-display text-[22px] font-bold tracking-tight">
            New organisation
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="space-y-1.5">
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-popover rounded-xl ring-1 ring-border shadow-lg z-10 max-h-48 overflow-y-auto">
                  {filteredUsers.map((u) => (
                    <button
                      ...
                      key={u._id}
                      onClick={() => {
                        setAdminUserId(u._id as Id<"users">);
            ...
            <p className="text-[11px] text-muted-foreground">
              {adminUserId ? "User will be added as an org admin" : "You can assign an admin later"}
            </p>
          </div>
