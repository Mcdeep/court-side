...
export function JoinQR({
  ...
  return (
    <div
      className={`inline-flex flex-col items-center gap-3 p-4 rounded-2xl ${tone === "dark" ? "bg-white/10" : "bg-zinc-50 ring-1 ring-zinc-200/80"}`}
    >
      <QRCodeSVG
        value={url}
        size={size}
      ...
      />
      <span
        className={`text-xs font-medium ${tone === "dark" ? "text-white/50" : "text-zinc-400"}`}
      >
        Scan to join
      </span>
    </div>
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-2xl p-8 max-w-xs w-full text-center"
          ...
          >
            ...
            <button
              onClick={() => setOpen(false)}
              className="mt-5 text-sm text-zinc-400 hover:text-zinc-600 font-medium"
            >
              Close
            </button>
          </div>
