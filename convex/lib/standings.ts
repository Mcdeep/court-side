...
export function rankStandings<
  Row extends { participantId: string; points: number; wins: number; losses: number },
>(rows: Row[], matches: StandingsMatch[], order: readonly Tiebreak[] = DEFAULT_TIEBREAK_ORDER) {
  const differences = new Map<string, number>();
  const games = new Map<string, number>();
  for (const match of matches) {
    for (const [players, difference] of [
      [match.pairA, match.scoreA - match.scoreB],
      [match.pairB, match.scoreB - match.scoreA],
    ] as const) {
      ...
      for (const row of group) {
        const value =
          criterion === "head_to_head"
            ? headToHead.get(row.participantId)!
            : criterion === "point_diff"
              ? row.pointDiff
              : row[criterion];
