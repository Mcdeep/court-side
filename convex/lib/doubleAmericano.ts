...
export const groupSplitModeValidator = v.union(
  v.literal("random"),
  v.literal("top_bottom"),
  v.literal("balanced"),
);
export const groupValidator = v.union(v.literal(1), v.literal(2));
export const isDoubleAmericano = (
  tournament: Pick<Doc<"tournaments">, "format" | "americanoVariant">,
) => tournament.format === "americano" && tournament.americanoVariant === "double";

export async function participantSkillRating(
...
export async function doubleFinalsComplete(
  ctx: Pick<QueryCtx, "db">,
  ...
  return !(
    ...
    matches.some(
      (match) =>
        ...
        match.finalMatchIndex < 0 ||
        match.finalMatchIndex > 3 ||
...
export async function assertDoubleFinalsComplete(
  ctx: Pick<QueryCtx, "db">,
  tournament: Doc<"tournaments">,
) {
...
export async function assertGroupResultsEditable(
  ctx: Pick<QueryCtx, "db">,
  tournament: Doc<"tournaments">,
  round: Doc<"rounds">,
) {
  ...
  const rounds = await ctx.db
    ...
    .withIndex("by_tournament", (q) => q.eq("tournamentId", tournament._id))
    .take(200);
  if (rounds.some((candidate) => candidate.stage === "final"))
    throw new Error(
      "Group results are locked. Reset crossover finals before changing a group result.",
    );
}
