...
const row = (participantId: string, points = 20, wins = 1) => ({ participantId, points, wins, losses: 1 })
const match = (pairA: string[], pairB: string[], scoreA: number, scoreB: number) => ({ pairA, pairB, scoreA, scoreB })
const placements = (rows: ReturnType<typeof rankStandings>) => rows.map(r => [r.participantId, r.rank, r.tied])

describe('rankStandings', () => {
  test('keeps points first and breaks equal points by wins', () => {
    expect(placements(rankStandings([row('a', 20, 1), row('b', 20, 2), row('c', 21, 0)], [])))
  ...
  test('breaks equal wins by point difference', () => {
    ...
    expect(ranked.map(r => [r.participantId, r.pointDiff, r.rank])).toEqual([['b', 8, 1], ['a', 2, 2]])
  })
  ...
  test('breaks a tie using only matches played as opponents', () => {
    ...
    expect(placements(ranked)).toEqual([['a', 1, false], ['b', 2, false]])
  })
  ...
  test('shares unresolved positions and skips occupied ranks', () => {
    expect(placements(rankStandings([row('d', 10), row('c'), row('b'), row('a', 30)], [])))
      .toEqual([['a', 1, false], ['b', 2, true], ['c', 2, true], ['d', 4, false]])
  })

  test('shares the title and produces stable ties without mutating its inputs', () => {
    const rows = [row('b'), row('a')]
    const original = structuredClone(rows)
    expect(placements(rankStandings(rows, []))).toEqual([['a', 1, true], ['b', 1, true]])
