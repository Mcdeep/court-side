...
export async function usesLegacyStandings(
...
export async function getTournamentStandings(
  ...
  if (double)
    for (const participant of participants)
      totals.set(participant._id, { points: 0, wins: 0, losses: 0 });
  if (useTiebreaks) {
    ...
    for (const [index, games] of roundMatches.entries()) {
      if (double && rounds[index].stage === "final") {
        finalMatches.push(...games);
        continue;
      }
  ...
  for (const row of rows) {
    const teamId = FIXED_PAIR_FORMATS.includes(tournament.format)
      ? participantById.get(row.participantId)?.teamId
      : undefined;
  ...
  const sorted = double
    ? ([1, 2] as const).flatMap((group) =>
        rankStandings(
          units.filter((row) => row.group === group),
          matches,
          order,
        ),
      )
  ...
  if (
    double &&
    finalMatches.length === 4 &&
    new Set(finalMatches.map((match) => match.finalMatchIndex)).size === 4 &&
    finalMatches.every(
      (match) =>
        match.state === "completed" &&
        match.finalMatchIndex !== undefined &&
        getRecordedScore(match) &&
    ...
    for (const match of finalMatches) {
      ...
      for (const id of [a.participantAId, a.participantBId])
        placements.set(id, base + Number(!aWins));
      for (const id of [b.participantAId, b.participantBId])
        placements.set(id, base + Number(aWins));
    }
  }
  const result = sorted.map((row) => ({
    ...
    ...(placements.has(row.participantId)
      ? { finalPlacement: placements.get(row.participantId) }
      : {}),
