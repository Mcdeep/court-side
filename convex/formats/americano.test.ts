...
function fixtures(rounds: RoundPlan[]) {
  return rounds.flat().map(match =>
    [match.pairA, match.pairB].map(pair => [...pair].sort().join(':')).sort().join('|'),
  ).sort()
}
...
describe('generateAmericanoRounds', { timeout: 15_000 }, () => {
  ...
  test.each([[8, 2], [12, 3], [16, 4], [16, 2], [16, 3], [20, 5], [24, 4], [36, 4], [40, 6]])(
    'preserves fixtures within each partnership round for %i players and %i courts', (players, courts) => {
      ...
      for (let index = 0; index < players - 1; index++) {
        expect(fixtures(balanced.slice(index * wavesPerRound, (index + 1) * wavesPerRound))).toEqual(
          fixtures(original.slice(index * matchesPerRound, (index + 1) * matchesPerRound)),
        )
      }
  ...
  test('proves the eight-player template cannot achieve court spread below five', () => {
    ...
    for (let mask = 0; mask < 2 ** rounds.length; mask++) {
      const assignment = rounds.map((round, index) => round.map(match => ({
        ...match,
  ...
  test('proves the twelve-player template cannot keep every court count between three and four', () => {
    ...
    const rounds = generateAmericanoRounds(participants, 3, seededRandom(42))
      .map(round => round.map(match => [...match.pairA, ...match.pairB].map(Number)))
    ...
    function search(index: number): boolean {
      ...
      for (const permutation of index === 0 ? [permutations[0]] : permutations) {
        ...
        if (round.some((match, slot) => match.some(player => counts[player][permutation[slot]] === 4))) continue
        round.forEach((match, slot) => match.forEach(player => counts[player][permutation[slot]]++))
        const remaining = rounds.length - index - 1
        if (counts.every(row => row.every(count => count + remaining >= 3)) && search(index + 1)) return true
        round.forEach((match, slot) => match.forEach(player => counts[player][permutation[slot]]--))
      }
      return false
    }
  ...
  test('preserves the sixteen-player group structure used to prove its court-spread bounds', () => {
    ...
    for (const round of rounds) {
      ...
      const key = groups.map(group => group.join(':')).sort().join('|')
