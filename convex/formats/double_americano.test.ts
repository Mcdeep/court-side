...
describe("splitDoubleAmericanoGroups", () => {
  ...
  test.each([undefined, Number.NaN, Number.POSITIVE_INFINITY, 0.9, 7.1])(
    "rejects invalid rated-mode rating %s",
    (rating) => {
  ...
  test("finds the exact minimum balanced partition with fractional ratings", () => {
    const ratings = [
      6.9, 6.4, 6.1, 5.8, 5.2, 4.9, 4.3, 4.1, 3.8, 3.6, 3.3, 2.9, 2.5, 2.2, 1.7, 1.1,
    ];
...
test("balanced splitting reaches the closest possible totals when equality is impossible", () => {
  ...
  const sum = (group: string[]) =>
    group.reduce((total, id) => total + players.find((player) => player.id === id)!.rating, 0);
...
describe("generateDoubleAmericanoRounds", () => {
  test.each([
    ...
    "assigns $courts courts across the expected waves",
    ({ courts, roundCount, matchesPerRound, usedCourts }) => {
  ...
  test.each([
    ...
    "swaps group courts after each complete partnership round with $courts courts",
    ({ courts, wavesPerRound, firstCourts, secondCourts }) => {
      ...
      for (let partnershipRound = 0; partnershipRound < 7; partnershipRound++) {
        const waves = rounds.slice(
          partnershipRound * wavesPerRound,
          (partnershipRound + 1) * wavesPerRound,
        );
        const expectedCourts =
          partnershipRound % 2 === 0 ? [firstCourts, secondCourts] : [secondCourts, firstCourts];
        for (const wave of waves) {
          ...
          for (const [groupIndex, group] of groups.entries()) {
            ...
            expect(groupMatches.map((match) => match.courtNumber)).toEqual(
              expectedCourts[groupIndex],
            );
            expect(
              groupMatches
                .flatMap((match) => [...match.pairA, ...match.pairB])
                .every((id) => group.includes(id)),
            ).toBe(true);
          }
        }
  ...
  test.each([1, 1.5, Number.NaN])("rejects invalid court count %s", (courtCount) => {
    expect(() =>
      generateDoubleAmericanoRounds(ids.slice(0, 8), ids.slice(8), courtCount),
    ).toThrow();
  });
});
...
describe("generateDoubleAmericanoFinals", () => {
  test.each([0, -1, 1.5, Number.NaN])("rejects invalid final court count %s", (courtCount) => {
    expect(() =>
      generateDoubleAmericanoFinals(ids.slice(0, 8), ids.slice(8), courtCount),
    ).toThrow();
  });
  test("creates the four crossover finals from ordered group seeds", () => {
    ...
    expect(finals).toEqual([
      [
        {
          pairA: ["player-1", "player-10"],
          pairB: ["player-9", "player-2"],
          courtNumber: 1,
          finalMatchIndex: 0,
        },
        {
          pairA: ["player-3", "player-12"],
          pairB: ["player-11", "player-4"],
          courtNumber: 2,
          finalMatchIndex: 1,
        },
        {
          pairA: ["player-5", "player-14"],
          pairB: ["player-13", "player-6"],
          courtNumber: 3,
          finalMatchIndex: 2,
        },
      ],
      [
        {
          pairA: ["player-7", "player-16"],
          pairB: ["player-15", "player-8"],
          courtNumber: 1,
          finalMatchIndex: 3,
        },
      ],
