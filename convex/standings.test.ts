  ...
  async function addMatch(a: number[], b: number[]) {
    return t.run(async (ctx) => {
      ...
      const pairAId = await ctx.db.insert("pairs", {
        tournamentId,
        participantAId: participants[a[0]],
        participantBId: participants[a[1]],
      });
      const pairBId = await ctx.db.insert("pairs", {
        tournamentId,
        participantAId: participants[b[0]],
        participantBId: participants[b[1]],
      });
...
describe("tournament tiebreak settings", () => {
  test("stores defaults on create and preserves a chosen order when duplicating", async () => {
    ...
    await organizer.mutation(api.tournaments.update, {
      tournamentId,
      tiebreakOrder: ["points", "head_to_head", "point_diff", "wins"],
    });
  ...
  test("rejects missing or repeated criteria", async () => {
    ...
    await expect(
      organizer.mutation(api.tournaments.update, {
        tournamentId,
        tiebreakOrder: ["wins", "wins", "point_diff"],
      }),
    ).rejects.toThrow("exactly once");
    await expect(
      organizer.mutation(api.tournaments.update, { tournamentId, tiebreakOrder: [] }),
    ).rejects.toThrow("exactly once");
    await expect(
      organizer.mutation(api.tournaments.update, {
        tournamentId,
        tiebreakOrder: ["wins", "point_diff", "head_to_head"],
      }),
    ).rejects.toThrow("exactly once");
  });
  ...
  test("locks the order after completion while allowing unrelated edits", async () => {
    ...
    await expect(
      organizer.mutation(api.tournaments.update, {
        tournamentId,
        tiebreakOrder: ["points", "point_diff", "wins", "head_to_head"],
      }),
...
describe("Americano standings and ratings", () => {
  test("a wins-first tournament ranks and awards players ahead of higher points totals", async () => {
    ...
    expect(standings.map((row) => row.players[0].displayName)).toEqual([
      "D",
      "A",
      "E",
      "F",
    ...
    expect(
      awards.find((award) => award.participantId === standings[0].participantId),
    ).toMatchObject({ placement: 1, pointsEarned: 10 });
    expect(
  ...
  test("existing three-criterion settings retain their effective order and completion lock", async () => {
    ...
    await t.run(async (ctx) =>
      ctx.db.patch(tournamentId, {
        tiebreakOrder: ["point_diff", "wins", "head_to_head"],
        state: "completed",
      }),
    );
    const standings = await t.query(api.leaderboard.get, { tournamentId });
  ...
  test("uses the saved order and current match results across standings and rating awards", async () => {
    ...
    expect(standings.find((row) => row.players[0].displayName === "A")).toMatchObject({
      pointDiff: 2,
      wins: 2,
      played: 2,
    });
    ...
    expect(
      (await t.query(api.leaderboard.get, { tournamentId })).map(
        (row) => row.players[0].displayName,
      ),
    ).toEqual(["E", "F", "D", "B", "A", "C"]);
    await t.run(async (ctx) => ctx.db.patch(tournamentId, { state: "completed" }));
    ...
    const awards = await t.run(async (ctx) => {
      ...
      return Promise.all(
        history.map(async (row) => ({
          name: (await ctx.db.get(row.memberId!))!.name,
    ...
    expect(awards).toEqual(
      expect.arrayContaining([
        { name: "E", placement: 1, points: 9 },
        { name: "F", placement: 1, points: 9 },
        { name: "B", placement: 4, points: 4 },
        { name: "A", placement: 5, points: 3 },
      ]),
    );
  });
  ...
  test("re-scoring updates rankings and awarded ratings without counting the tournament twice", async () => {
    ...
    const awards = await t.run(async (ctx) => {
      const history = await ctx.db
        ...
        .collect();
      return Promise.all(
        history.map(async (row) => ({ ...row, member: await ctx.db.get(row.memberId!) })),
      );
    });
    expect(awards).toHaveLength(6);
    expect(awards.find((row) => row.member?.name === "B")).toMatchObject({
      placement: 2,
      pointsEarned: 8,
      member: { startingPoints: 8, tournamentsPlayed: 1 },
    });
    await t.mutation(internal.ratings.awardRatings, { tournamentId });
    expect(
      await t.run(async (ctx) =>
  ...
  test("score corrections retain the rating tiers used when the tournament finished", async () => {
    ...
    await t.run(async (ctx) => ctx.db.patch(tournamentId, { state: "completed" }));
    await t.mutation(internal.ratings.awardRatings, { tournamentId });
    await organizer.mutation(api.ratings.setTiers, {
      organizationId,
      tiers: [100, 80, 60, 40, 30, 20],
    });
    await t.mutation(internal.ratings.awardRatings, { tournamentId });
    const awards = await t.run(async (ctx) =>
      ctx.db
    ...
    expect(awards.map((award) => award.pointsEarned).sort((a, b) => b - a)).toEqual([
      9, 9, 6, 4, 3, 2,
    ]);
  });
  ...
  test("score corrections follow a member who links an account after the tournament", async () => {
    vi.useFakeTimers();
    const { t, organizer, organizationId, participants, tournamentId, scoreFixtures } =
      await setup();
    const matchIds = await scoreFixtures();
    await t.run(async (ctx) => ctx.db.patch(tournamentId, { state: "completed" }));
    await t.mutation(internal.ratings.awardRatings, { tournamentId });
    ...
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    const rating = await t.run(async (ctx) =>
      ctx.db
        .query("playerRatings")
  ...
  test("approved and admin-resolved scores persist the final match scores", async () => {
    ...
    const matchId = await addMatch([0, 1], [2, 3]);
    await organizer.mutation(api.scores.submit, {
      matchId,
      submittedBy: participants[0],
      scoreA: 10,
      scoreB: 8,
    });
    await organizer.mutation(api.scores.submit, {
      matchId,
      submittedBy: participants[2],
      scoreA: 10,
      scoreB: 8,
    });
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    expect(await t.run(async (ctx) => ctx.db.get(matchId))).toMatchObject({
      state: "completed",
      scoreA: 10,
      scoreB: 8,
    });
  ...
  test("does not mistake approved submissions for an older admin-resolved result", async () => {
    ...
    expect(
      (await t.query(api.leaderboard.get, { tournamentId })).find(
        (row) => row.participantId === participants[1],
      ),
    ).toMatchObject({ points: 20, pointDiff: null, tiebreaksUnavailable: true });
    await expect(
      organizer.mutation(api.scores.saveResult, { matchId: matchIds[3], scoreA: 20, scoreB: 0 }),
    ).rejects.toThrow("previous result");
    const cached = await t.run(async (ctx) =>
      ctx.db
        .query("leaderboard")
        .withIndex("by_tournament_and_participant", (q) =>
          q.eq("tournamentId", tournamentId).eq("participantId", participants[1]),
        )
        .unique(),
    );
  ...
  test("finishing with a missing legacy result awards ratings from cached points", async () => {
    ...
    await t.run(async (ctx) =>
      ctx.db.patch(tournamentId, { state: "in_progress", tiebreakOrder: undefined }),
    );
    ...
    expect(awards).toHaveLength(6);
    expect(awards.find((award) => award.participantId === participants[1])).toMatchObject({
      placement: 3,
      pointsEarned: 3.75,
    });
  });
  ...
  test.each([
    ...
    "repairs and corrects a missing %s result without double-counting standings",
    async (format) => {
      vi.useFakeTimers();
      const { t, organizer, tournamentId, participants, scoreFixtures } = await setup();
      const matchIds = await scoreFixtures();
      ...
      await organizer.mutation(api.scores.saveResult, {
        ...
        scoreA: 20,
        scoreB: 0,
        previousScore: { scoreA: 10, scoreB: 0 },
      });
      await t.finishAllScheduledFunctions(vi.runAllTimers);
      expect(await t.run(async (ctx) => ctx.db.get(matchIds[3]))).toMatchObject({
        scoreA: 20,
        scoreB: 0,
      });
      const cached = await t.run(async (ctx) =>
  ...
  test("admin resolution can repair a missing result despite conflicting old submissions", async () => {
    ...
    await organizer.mutation(api.scores.resolve, {
      matchId: matchIds[3],
      scoreA: 8,
      scoreB: 12,
      previousScore: { scoreA: 10, scoreB: 0 },
  ...
  test("repairing a finished event replaces fallback awards with its configured ranking", async () => {
    ...
    const member = await t.run(async (ctx) =>
      ctx.db.get((await ctx.db.get(participants[1]))!.memberId!),
    );
  ...
  test("a partial repair updates cached standings before fallback awards are scheduled", async () => {
    ...
    const matchIds = await scoreFixtures();
    await t.run(async (ctx) => {
      for (const matchId of [matchIds[0], matchIds[3]])
        await ctx.db.patch(matchId, { scoreA: undefined, scoreB: undefined });
      await ctx.db.patch(tournamentId, { state: "in_progress" });
    });
    await organizer.mutation(api.tournaments.finish, { tournamentId });
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    ...
    const standings = await t.query(api.leaderboard.get, { tournamentId });
    expect(standings.find((row) => row.participantId === participants[1])).toMatchObject({
      points: 30,
      rank: 1,
      tiebreaksUnavailable: true,
    });
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    const awards = await t.run(async (ctx) => ctx.db.query("ratingHistory").collect());
    expect(awards.find((award) => award.participantId === participants[1])).toMatchObject({
  ...
  test("stored match scores take precedence over a stale restoration entry", async () => {
    ...
    await organizer.mutation(api.scores.saveResult, {
      ...
      scoreA: 20,
      scoreB: 0,
      previousScore: { scoreA: 99, scoreB: 99 },
    });
    await t.finishAllScheduledFunctions(vi.runAllTimers);
    const cached = await t.run(async (ctx) =>
      ctx.db
  ...
  test("restoration requires the existing score permissions and valid previous scores", async () => {
    ...
    await expect(t.mutation(api.scores.saveResult, correction)).rejects.toThrow(
      "Not authenticated",
    );
    await expect(
      organizer.mutation(api.scores.saveResult, {
        ...correction,
        previousScore: { scoreA: -1, scoreB: 0 },
      }),
  ...
  test("completed legacy events keep points-only ranks and awards, even after reopening", async () => {
    ...
    await scoreFixtures();
    await t.run(async (ctx) =>
      ctx.db.patch(tournamentId, { state: "completed", tiebreakOrder: undefined }),
    );
    const standings = await t.query(api.leaderboard.get, { tournamentId });
  ...
  test("reopening a historical event without rating history preserves points-only standings", async () => {
    ...
    expect((await t.query(api.leaderboard.get, { tournamentId })).map((row) => row.rank)).toEqual([
      1, 1, 3, 3, 3, 3,
    ]);
  });
  ...
  test("fixed pairs remain a single placement unit with points-only tie averaging", async () => {
    ...
    const standings = await t.query(api.leaderboard.get, { tournamentId });
    expect(standings.map((row) => [row.rank, row.players.length, row.pointDiff])).toEqual([
      [1, 2, null],
      [1, 2, null],
      [3, 2, null],
    ]);
